...
class ChompSolver:
    ...
    def calculate_total_gradient(self, xi, T, robot, params, M, A, b_vec, c, lambda_, DOF):
        ...
        ~= MATLAB total_gradient(...)
        """
        dT = params.dT
        Nint = T - 2

        # internal points only
        xi_int = xi[1:-1, :]        # (Nint x DOF)
        xi_int_vec = xi_int.T.reshape(-1)  # DOF-major, matches A

        # Smoothness cost
        # Fsmooth = 0.5 * xi_int' * A * xi_int + b' * xi_int + c
        Fsmooth_cost = 0.5 * xi_int_vec.T @ A @ xi_int_vec + np.dot(b_vec, xi_int_vec) + c
        nabla_Fsmooth_vec = A @ xi_int_vec + b_vec  # gradient of smoothness wrt xi_int_vec

        # Obstacle functional gradient (and cost)
        nabla_Fobs_vec, Fobs_cost = self.chompFobs(xi, robot, M, dT, DOF)
...
if __name__ == "__main__":
    ...
    for d in range(params.DOF):
        xi0[:, d] = np.linspace(q_start[d], q_goal[d], T)

    #plot
    plt.ion()
    h_image_metric = ch.plotMap(M)
    plt.figure(h_image_metric.number)
    #plot initial trajectory
    plt.plot(xi0[:, 0], xi0[:, 1], "ko-", linewidth=2, label="Initial")
    #optimize
    optimized_xi = ch.chompSolve(params, robot, M, xi0, h_image_metric)
    plt.figure(h_image_metric.number)
    ...
    #
    # plt.figure()
    # plt.plot(theta_des, "bo-", linewidth=2, label="TANGENT")
    # plt.plot(optimized_xi[:, 2], "ro-", linewidth=2, label="CHOMP")
    # plt.grid(True)
